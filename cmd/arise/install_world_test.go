...
func TestLiveMutationWorldJournalGate(t *testing.T) {
	tests := []struct {
		...
		{name: "named package set does not enter world", targets: []string{"@preserved-rebuild"}, want: false},
	}
...
func TestTargetsNeedCompleteGraph(t *testing.T) {
	for _, targets := range [][]string{{"@world"}, {"@system"}, {"dev-lang/go", "@world"}} {
		if !targetsNeedCompleteGraph(targets) {
			t.Fatalf("targetsNeedCompleteGraph(%v) = false", targets)
		}
	}
	if targetsNeedCompleteGraph([]string{"dev-lang/go"}) {
		t.Fatal("explicit package unexpectedly enabled set complete-graph repair")
	}
}
