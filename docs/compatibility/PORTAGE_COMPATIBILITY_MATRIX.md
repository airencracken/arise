...
| `perl-cleaner --modules/--allmodules/--libperl/--all/--reallyall` | `arise perl-cleaner` with the same mode | supported subset | VDB-derived ABI works without Perl/Python runtimes; typed module/libperl evidence, perl-core/virtual preclean, resolver-backed repair, independent final audit, journaled known leftovers, unknown-leftover reporting, mode/adversarial/fuzz tests and live `--modules` differential; wider libperl/leftover differential corpus remains |
