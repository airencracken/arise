...
| no direct Portage equivalent | `arise python-cleaner --check/--pretend` | foundation | effective target/single-target policy, ordered python-exec preference, installed slots, recorded package targets and dependencies, owned paths, strict libpython linkage, confined shebang probes, bounded orphan inventory, removal blockers, versioned JSON, unavailable-ebuild reporting, exact-version-aware dependency-connected recovery cohorts, and resolver-backed pretend validation; mutation remains gated pending runtime probes, resumable application, and atomic preference switching |
