...
| `/var/lib/portage/world` | supported subset | locked atomic deselect and explicit world maintenance preserve mode/ownership; optional saved-plan approval remains state-bound; constrained moves, provably redundant constraints, concurrent-change rejection, interruption safety, alternate roots and disposable-root Portage repair parity are covered |
...
| `perl-cleaner --modules/--allmodules/--libperl/--all/--reallyall` | `arise perl-cleaner` with the same mode; `--resume` | supported | VDB-derived ABI works without Perl/Python runtimes; typed module/libperl evidence, perl-core/virtual preclean, resolver-backed repair, integrity-bound resumable context plus per-package progress, independent final audit, exact known-leftover policy and journaled deletion, unknown-leftover reporting, mode/adversarial/fuzz/rollback tests, live-host modules parity, and bubblewrap-isolated differentials for every mode union, preclean, libperl, and leftovers |
| no direct Portage equivalent | `arise python-cleaner --check/--pretend/--fix/--resume` | supported recovery subset | effective target/single-target policy, ordered python-exec preference, installed slots, recorded package targets and dependencies, owned paths, strict libpython linkage, confined shebang probes, bounded orphan inventory, removal blockers, versioned JSON, unavailable-ebuild reporting, exact-version-aware dependency-connected recovery cohorts, resolver-backed pretend validation, integrity-bound cohort checkpoints, post-transaction re-inventory, bounded direct-interpreter and native-extension probes, and atomic python-exec publication; unavailable packages block publication, and interpreter/orphan removal remains report-only |
