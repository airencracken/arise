...
func ReadInfo(path string) (*BinPkgInfo, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("binpkg: stat %s: %w", path, err)
	}

	f, err := os.Open(path)
	if err != nil {
		return nil, fmt.Errorf("binpkg: open %s: %w", path, err)
	}
	defer f.Close()

	meta, err := readXPAKMetadata(f, fi.Size())
	if err != nil {
		return nil, fmt.Errorf("binpkg: reading metadata from %s: %w", path, err)
	}
...
func readXPAKMetadata(f *os.File, size int64) (map[string]string, error) {
	...
	if readSize == 0 {
		return nil, fmt.Errorf("binpkg: empty file")
	}

	buf := make([]byte, readSize)
	if _, err := f.ReadAt(buf, size-readSize); err != nil {
		return nil, fmt.Errorf("binpkg: reading trailer: %w", err)
	}

	idx := bytes.LastIndex(buf, []byte(xpakMagic+"\n"))
	if idx < 0 {
		return nil, fmt.Errorf("binpkg: XPAKSTOP not found in trailer")
	}

	afterMagic := buf[idx+len(xpakMagic)+1:]
	nlIdx := bytes.IndexByte(afterMagic, '\n')
	if nlIdx < 0 {
		return nil, fmt.Errorf("binpkg: malformed XPAK trailer (no offset)")
	}

	offsetStr := string(afterMagic[:nlIdx])
	offset, err := strconv.ParseInt(offsetStr, 10, 64)
	if err != nil {
		return nil, fmt.Errorf("binpkg: malformed XPAK offset %q: %w", offsetStr, err)
	}
	if offset > size || offset < 0 {
		return nil, fmt.Errorf("binpkg: XPAK offset %d out of range (file size %d)", offset, size)
	}
	...
	if _, err := f.ReadAt(metaData, metaStart); err != nil {
		return nil, fmt.Errorf("binpkg: reading metadata: %w", err)
	}
...
func Extract(ctx context.Context, pkgPath string, destDir string) error {
	...
	if err != nil {
		return fmt.Errorf("binpkg: open %s: %w", pkgPath, err)
	}
	defer f.Close()

	fi, err := f.Stat()
	if err != nil {
		return fmt.Errorf("binpkg: stat %s: %w", pkgPath, err)
	}

	tarStart, err := findXPAKStart(f, fi.Size())
	if err != nil {
		return fmt.Errorf("binpkg: finding XPAK boundary in %s: %w", pkgPath, err)
	}

	if _, err := f.Seek(0, io.SeekStart); err != nil {
		return fmt.Errorf("binpkg: seek: %w", err)
	}
...
func extractWithXz(ctx context.Context, pkgPath, destDir string, tarStart int64) error {
	...
	if err := cmd.Run(); err != nil {
		return fmt.Errorf("binpkg: xz decompress: %w", err)
	}
...
func findXPAKStart(f *os.File, size int64) (int64, error) {
	...
	buf := make([]byte, readSize)
	if _, err := f.ReadAt(buf, size-readSize); err != nil {
		return 0, fmt.Errorf("binpkg: reading trailer: %w", err)
	}
...
func untar(r io.Reader, destDir string) error {
	...
	for {
		...
		if err != nil {
			return fmt.Errorf("binpkg: tar read: %w", err)
		}
		...
		case tar.TypeDir:
			if err := os.MkdirAll(target, os.FileMode(hdr.Mode)&0777); err != nil {
				return fmt.Errorf("binpkg: mkdir %s: %w", target, err)
			}
		case tar.TypeReg:
			if err := os.MkdirAll(filepath.Dir(target), 0755); err != nil {
				return fmt.Errorf("binpkg: mkdir parent: %w", err)
			}
			f, err := os.OpenFile(target, os.O_CREATE|os.O_WRONLY|os.O_TRUNC, os.FileMode(hdr.Mode)&0777)
			if err != nil {
				return fmt.Errorf("binpkg: create %s: %w", target, err)
			}
			if _, err := io.Copy(f, tr); err != nil {
				f.Close()
				return fmt.Errorf("binpkg: write %s: %w", target, err)
			}
			f.Close()
		case tar.TypeSymlink:
			...
			if err := os.Symlink(hdr.Linkname, target); err != nil {
				return fmt.Errorf("binpkg: symlink %s -> %s: %w", target, hdr.Linkname, err)
			}
		}
	}
...
func Create(ctx context.Context, vdbEntryPath string, rootDir string, pkgDir string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("binpkg: reading VDB metadata: %w", err)
	}

	entries, err := parseContents(filepath.Join(vdbEntryPath, "CONTENTS"))
	if err != nil {
		return "", fmt.Errorf("binpkg: parsing CONTENTS: %w", err)
	}
	...
	if err := os.MkdirAll(outDir, 0755); err != nil {
		return "", fmt.Errorf("binpkg: create pkg dir: %w", err)
	}
	...
	if err != nil {
		return "", fmt.Errorf("binpkg: create temp: %w", err)
	}
	...
	for _, entry := range entries {
		...
		case "dir":
			...
			if err := tw.WriteHeader(hdr); err != nil {
				...
				bzWriter.Close()
				tmpF.Close()
				return "", fmt.Errorf("binpkg: write tar dir header for %s: %w", entry.Path, err)
			}
		case "obj":
			...
			if err != nil {
				...
				bzWriter.Close()
				tmpF.Close()
				return "", fmt.Errorf("binpkg: stat %s: %w", srcPath, err)
			}
			if fi.Mode()&os.ModeSymlink != 0 {
				...
				if err != nil {
					...
					bzWriter.Close()
					tmpF.Close()
					return "", fmt.Errorf("binpkg: readlink %s: %w", srcPath, err)
				}
				hdr := &tar.Header{
					Name:     entry.Path,
				...
				if err := tw.WriteHeader(hdr); err != nil {
					...
					bzWriter.Close()
					tmpF.Close()
					return "", fmt.Errorf("binpkg: write tar symlink header: %w", err)
				}
				continue
			}
			hdr := &tar.Header{
			...
			if err := tw.WriteHeader(hdr); err != nil {
				...
				bzWriter.Close()
				tmpF.Close()
				return "", fmt.Errorf("binpkg: write tar file header: %w", err)
			}
			...
			if err != nil {
				...
				bzWriter.Close()
				tmpF.Close()
				return "", fmt.Errorf("binpkg: open %s: %w", srcPath, err)
			}
			if _, err := io.Copy(tw, srcF); err != nil {
				...
				bzWriter.Close()
				tmpF.Close()
				return "", fmt.Errorf("binpkg: copy %s: %w", srcPath, err)
			}
			srcF.Close()
		case "sym":
			link := entry.Target
	...
	if err := tw.Close(); err != nil {
		bzWriter.Close()
		tmpF.Close()
		return "", fmt.Errorf("binpkg: tar close: %w", err)
	}
	if err := bzWriter.Close(); err != nil {
		tmpF.Close()
		return "", fmt.Errorf("binpkg: bzip2 close: %w", err)
	}
	...
	if _, err := tmpF.Write(xpakMeta); err != nil {
		tmpF.Close()
		return "", fmt.Errorf("binpkg: write XPAK metadata: %w", err)
	}
	...
	if _, err := tmpF.WriteString(trailer); err != nil {
		tmpF.Close()
		return "", fmt.Errorf("binpkg: write XPAK trailer: %w", err)
	}

	if err := tmpF.Close(); err != nil {
		return "", fmt.Errorf("binpkg: close temp: %w", err)
	}

	if err := os.Rename(tmpPath, outPath); err != nil {
		return "", fmt.Errorf("binpkg: rename %s -> %s: %w", tmpPath, outPath, err)
	}
...
func ListAvailable(pkgDir string) ([]*BinPkgInfo, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("binpkg: read pkg dir: %w", err)
	}
...
func FindPackage(pkgDir string, atomStr string) (*BinPkgInfo, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("binpkg: parse atom %q: %w", atomStr, err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("binpkg: parse version %q: %w", a.Version.Raw, err)
	}
...
func DownloadFromBinhost(ctx context.Context, binhostURL string, atomStrs []string, destDir string) ([]string, error) {
	if err := os.MkdirAll(destDir, 0755); err != nil {
		return nil, fmt.Errorf("binpkg: create dest dir: %w", err)
	}
	...
	for _, atomStr := range atomStrs {
		...
		if err := os.MkdirAll(catDir, 0755); err != nil {
			return downloaded, fmt.Errorf("binpkg: create category dir %s: %w", catDir, err)
		}
		...
		if err != nil {
			return downloaded, fmt.Errorf("binpkg: create request %s: %w", url, err)
		}

		resp, err := httpClient.Do(req)
		if err != nil {
			return downloaded, fmt.Errorf("binpkg: request %s: %w", url, err)
		}

		if resp.StatusCode < 200 || resp.StatusCode >= 300 {
			resp.Body.Close()
			return downloaded, fmt.Errorf("binpkg: HTTP %d for %s", resp.StatusCode, url)
		}
		...
		if err != nil {
			resp.Body.Close()
			return downloaded, fmt.Errorf("binpkg: create temp file: %w", err)
		}
		...
		if _, err := io.Copy(fh, resp.Body); err != nil {
			...
			resp.Body.Close()
			os.Remove(tmpPath)
			return downloaded, fmt.Errorf("binpkg: download %s: %w", url, err)
		}
		fh.Close()
		resp.Body.Close()
		...
		if err := os.Rename(tmpPath, destPath); err != nil {
			os.Remove(tmpPath)
			return downloaded, fmt.Errorf("binpkg: rename temp file: %w", err)
		}
...
func newBzip2Writer(w io.Writer) io.WriteCloser {
	...
	if err != nil {
		return &brokenWriter{err: fmt.Errorf("binpkg: bzip2 stdin pipe: %w", err)}
	}
	if err := cmd.Start(); err != nil {
		return &brokenWriter{err: fmt.Errorf("binpkg: bzip2 start: %w", err)}
	}
