...
func RebuildPackage(ctx context.Context, atomStr string, cfg RebuildConfig) (err error) {
	...
	if err != nil {
		return fmt.Errorf("rebuild: parse atom %q: %w", atomStr, err)
	}

	if a.Version == nil || a.Version.Raw == "" {
		return fmt.Errorf("rebuild: atom %q must include a version", atomStr)
	}
	...
	if err != nil {
		return fmt.Errorf("rebuild: find ebuild for %s/%s-%s: %w", cat, pkg, ver, err)
	}

	eb, err := ebuild.ParseEbuild(ebuildFile)
	if err != nil {
		return fmt.Errorf("rebuild: parse ebuild %s: %w", ebuildFile, err)
	}
	...
	if err != nil {
		return fmt.Errorf("rebuild: create workdir: %w", err)
	}
	defer func() {
	...
	if err != nil {
		return fmt.Errorf("rebuild: create destdir: %w", err)
	}
	...
	if len(resolvedURIs) > 0 {
		...
		if _, err := fetch.Fetch(ctx, resolvedURIs, fetchCfg); err != nil {
			cfg.fireError(atomStr, fmt.Errorf("fetch: %w", err))
			return fmt.Errorf("rebuild: fetch sources: %w", err)
		}
	}
	...
	if err != nil {
		cfg.fireError(atomStr, fmt.Errorf("runner: %w", err))
		return fmt.Errorf("rebuild: create runner: %w", err)
	}
	...
	for _, ph := range buildPhases {
		...
		if err != nil {
			cfg.fireError(atomStr, fmt.Errorf("phase %s: %w", ph, err))
			return fmt.Errorf("rebuild: phase %s: %w", ph, err)
		}
	}
	...
	if err := merge.Merge(ctx, destDir, mergeCfg); err != nil {
		cfg.fireError(atomStr, fmt.Errorf("merge: %w", err))
		return fmt.Errorf("rebuild: merge: %w", err)
	}
	...
	if cfg.Features != nil && cfg.Features.IsEnabled(features.FeatBuildPkg) {
		...
		if _, bpkgErr := exec.LookPath("bzip2"); bpkgErr == nil {
			binpkg.Create(ctx, mergeCfg.VdbPath(), cfg.RootDir, pkgDir)
		}
	}
...
func findEbuild(repoDir, category, pkgName, version string) (string, error) {
	...
	if err != nil {
		if os.IsNotExist(err) {
			return "", fmt.Errorf("ebuild directory not found: %s", catDir)
		}
		return "", err
	}
	...
	if len(matches) == 0 {
		return "", fmt.Errorf("no ebuild found for %s-%s in %s", pkgName, version, catDir)
	}
...
func joinErrors(errs []error) error {
	...
	return fmt.Errorf("rebuild: %d error(s):\n%s", len(errs), strings.Join(msgs, "\n"))
}
